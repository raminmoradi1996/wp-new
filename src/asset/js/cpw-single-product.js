jQuery(document).ready(($) => {
    ...
    // click open modal 
    $('.stz-cpw-item-cart-body').on('click', (e) => {

        idCart = e.currentTarget.id
        // const type = $(`#${idCart}`).prop('type')
        console.log(idCart)

        $(`.stz-cpw-item-cart-body img`).css('opacity', '1')
        $(`.stz-cpw-item-cart-body .icon-select`).hide()
        $(`.stz-cpw-item-cart-body .select-checkbox`).prop('checked', false)

        $(`#${idCart} img`).css('opacity', '0.3')
        $(`#${idCart} .icon-select`).show()
        $(`#${idCart} .select-checkbox`).prop('checked', true)


        $('#stz-cpw-body-modal-main').hide()
        $('#stz-cpw-body-modal-info-cart').show()

    })


    $('.stz-cpw-item-cart-body-info-cart').on('click', (e) => {
